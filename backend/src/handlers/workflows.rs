...
pub async fn index(pool: web::Data<DbPool>, req: HttpRequest) -> HttpResponse {
    ...
    let mut stmt = conn.prepare("SELECT * FROM workflows WHERE organization_id = ?1 ORDER BY created_at DESC").unwrap();
    let items: Vec<Workflow> = stmt.query_map([org_id], Workflow::from_row);
...
pub async fn store(pool: web::Data<DbPool>, req: HttpRequest, body: web::Json<crate::models::workflow::CreateWorkflowRequest>) -> HttpResponse {
    ...
    match conn.execute(
        ...
        crate::params![body.name, body.description, body.trigger_type, trigger_conditions_str, actions_str, is_active, user_id, org_id, &now, &now],
    ) {
...
pub async fn update(pool: web::Data<DbPool>, req: HttpRequest, path: web::Path<i64>, body: web::Json<crate::models::workflow::CreateWorkflowRequest>) -> HttpResponse {
    ...
    match conn.execute(
        ...
        crate::params![body.name, body.description, body.trigger_type, trigger_conditions_str, actions_str, is_active, &now, path.into_inner(), org_id],
    ) {
