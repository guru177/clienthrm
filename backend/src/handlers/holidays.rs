...
pub async fn store(pool: web::Data<DbPool>, req: HttpRequest, body: web::Json<crate::models::holiday::CreateHolidayRequest>) -> HttpResponse {
    ...
    match conn.execute("INSERT INTO holidays (name,date,description,created_at,updated_at) VALUES (?1,?2,?3,?4,?5)",
        rusqlite::params![body.name, body.date, body.description, &now, &now]) {
