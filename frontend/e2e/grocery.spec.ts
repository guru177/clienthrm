...
test.describe('Grocery Benefits Workflow', () => {
    test.beforeEach(async ({ authenticatedPage: page }) => {
        await page.route('**/api/auth/me', async (route) => {
            ...
            if (json?.data) {
                ...
                json.data.plan.modules = Array.from(new Set([...(json.data.plan.modules || []), 'grocery_benefits', 'my_grocery_benefits']));
        ...
        await page.route('**/api/admin/grocery-benefits', async (route) => {
            await route.fulfill({
                ...
                json: {
                    success: true,
                    data: [
                        { id: 1, user_id: 2, user_name: 'Test Employee', start_date: '2026-07-01', subsidy_percentage: 50, monthly_allowance: 5000, status: 'active' }
                    ]
                }
            });
        });
        await page.route('**/api/admin/grocery-claims', async (route) => {
            await route.fulfill({
                ...
                json: {
                    success: true,
                    data: [
                        { id: 1, user_id: 2, user_name: 'Test Employee', claim_month: 7, claim_year: 2026, amount: 2000, company_share: 1000, employee_share: 1000, is_free_month: 0, status: 'pending' }
                    ]
                }
            });
        });
    ...
    test('admin can enroll employee', async ({ authenticatedPage: page }) => {
        await page.route('**/api/admin/users/list*', async (route) => {
            await route.fulfill({
                status: 200,
                json: { success: true, data: { data: [{ id: 3, name: 'New Hire', email: 'new@example.com', date_of_joining: '2026-07-15' }] } }
            });
        });
        
        await page.goto('/admin/grocery-benefits');
        await dismissOverlays(page);
        await page.getByRole('button', { name: /Enroll Employee/ }).click();
        ...
        await page.route('**/api/admin/grocery-benefits', async (route) => {
            if (route.request().method() === 'POST') {
                ...
                await route.fulfill({ status: 200, json: { success: true, message: 'Enrolled' } });
            } else {
    ...
    test('employee can view their benefits and submit a claim', async ({ authenticatedPage: page }) => {
        await page.route('**/api/admin/grocery-benefits/my-status', async (route) => {
            await route.fulfill({
                ...
                json: {
                    success: true,
                    data: {
                        benefit: { id: 1, start_date: '2026-07-01', subsidy_percentage: 50, monthly_allowance: 5000, status: 'active' },
                        is_free_month: false,
        ...
        await dismissOverlays(page);
        await expect(page.getByText('50% Grocery Subsidy Active')).toBeVisible();
        ...
        await page.route('**/api/admin/grocery-claims', async (route) => {
            if (route.request().method() === 'POST') {
                ...
                await route.fulfill({ status: 200, json: { success: true, message: 'Claim submitted' } });
            } else {
                await route.fallback();
            }
        });
