...
import { Plus, Users, Building2, CheckCircle, XCircle } from 'lucide-react';
...
export default function DepartmentsIndex() {
    ...
    const fetchStats = async () => {
        try {
            const response = await axios.get('/admin/departments/stats');
            ...
            <div className="space-y-6">
                    ...
                    <div className="relative flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div className="flex items-center gap-4">
                            <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-[#071b3a]/15 dark:bg-white/10 border border-[#071b3a]/20 dark:border-white/10 shadow-inner">
                                <Building2 className="h-6 w-6 text-[#071b3a] dark:text-blue-300" />
                            </div>
                            <div>
                ...
                <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
                    {statCards.map(({ label, key, icon: Icon, iconBg, valueColor, shadow }) => (
                        <div
                            key={key}
                            className="relative overflow-hidden rounded-2xl bg-white/70 dark:bg-white/5 backdrop-blur-md border border-white/80 dark:border-white/10 shadow-[0_4px_24px_rgba(7,27,58,0.06)] dark:shadow-[0_4px_24px_rgba(0,0,0,0.2)] p-5 group hover:-translate-y-0.5 transition-transform duration-200"
                        >
                            {/* Shimmer */}
