...
export default function DoctorReportsEdit() {
    ...
    const loadReport = async () => {
        ...
        try {
            ...
            setExistingPrescription(r.prescription_path);
        } catch (error) {
            handleApiError(error);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setSaving(true);
        try {
            await axios.put(`/admin/doctor-reports/${id}`, {
            ...
            // Upload new prescription if selected
            if (prescriptionFile) {
                setUploading(true);
                const fd = new FormData();
                fd.append('file', prescriptionFile);
                await axios.post(`/admin/doctor-reports/${id}/prescription`, fd, {
    ...
    if (loading) {
        return (
            <AppLayout breadcrumbs={[{ title: 'Doctor Reports', href: '/admin/doctor-reports' }, { title: 'Edit', href: '#' }]}>
        ...
        >
            <div className="space-y-6 max-w-4xl">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold">Edit SOAP Report</h1>
                        <p className="text-muted-foreground text-sm">Update consultation details</p>
                    </div>
                </div>
                            ...
                            {existingPrescription && (
                                <div className="rounded-md border p-3 bg-muted/50">
                                    <p className="text-sm text-muted-foreground">
                                        Current file: <span className="font-medium text-foreground">{existingPrescription.split('/').pop()}</span>
                                    </p>
                                </div>
                            )}
                            <div className="space-y-2">
                                <Label htmlFor="prescription_file">
                                    {existingPrescription ? 'Replace Prescription File' : 'Prescription File'} (PDF, JPG, PNG — max 10MB)
                                </Label>
                                <Input
