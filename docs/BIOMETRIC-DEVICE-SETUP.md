...
Step-by-step guide for HR admins and IT staff to connect attendance devices to Raintech HRM.

**Native protocols:** ZKTeco iClock/ADMS and BIO-PARK. **All other brands:** register the device serial + map PINs, then push punches via the authenticated Punch Push API (or try eSSL on ADMS port 7788 first).
...
| Physical device | ZKTeco / BIO-PARK (native), or any brand via Push API |
...
| Brand / protocol | Transport | Port / endpoint |
|------------------|-----------|-----------------|
| ZKTeco iClock | HTTP `/iclock/*` | 7788 |
| BIO-PARK ADMS | HTTP `/pub/chat`, `/pub/getrequest` | 7788 |
| BIO-PARK TCP (optional) | Binary TCP | 5010 |
| **Any other brand** (eSSL, Hikvision, Suprema, Matrix, …) | HTTP Push API | Main API `/api/integrations/biometric/punches` |

### Native vs Push API

- **Native (ZKTeco / BIO-PARK):** Point the device at the biometric port (**7788**) as described above.
- **eSSL:** Many units speak ZK ADMS/iClock — try port **7788** first. If the device does not push to iClock, use the Push API.
- **Hikvision / Suprema / Matrix / others:** There is no native ISAPI/SDK driver in HRM. Use vendor PC software, n8n, Postman, or a small bridge that POSTs punches to the Push API.

### Generic Punch Push API

1. In **Admin → Biometric → Devices**, register a serial number for the physical unit (any unique string you will send as `device_serial`).
2. Map device PINs to employees (**PIN Mapping**).
3. Open **Push API**, create an ingest key (shown once). Auth header: `Authorization: Bearer hrm_bio_…` or `X-Biometric-Key: hrm_bio_…`.
4. POST to the **main API** (port 3001 / your HTTPS app), not 7788:

```bash
curl -X POST "https://app.yourcompany.com/api/integrations/biometric/punches" \
  -H "Authorization: Bearer hrm_bio_YOUR_KEY" \
  -H "Content-Type: application/json" \
  -d '{"device_serial":"SN001","device_pin":"1","punch_time":"2026-07-17 09:00:00","punch_type":0}'
```

| Field | Required | Notes |
|-------|----------|--------|
| `device_serial` | Yes | Must be registered to the same org as the key |
| `device_pin` | Yes | Mapped via PIN Mapping for attendance |
| `punch_time` | Yes | `YYYY-MM-DD HH:MM:SS` |
| `punch_type` | No | `0` = in, `1` = out; omit to use HRM resolve logic |

Batch: send a JSON array, or `{ "punches": [ {...}, {...} ] }`.

Wrong key, revoked key, or a serial belonging to another org → rejected. Successful pushes update device heartbeat and appear in Punch Log like native punches.
