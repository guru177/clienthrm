...
from test_helpers import db_connect,  leave_reason, MIN_LEAVE_REASON
...
def ensure_open_employment(conn: sqlite3.Connection, user_id: int) -> None:
    """Clear exit date / on-leave so far-future workflow leave dates are valid."""
    conn.execute(
        """UPDATE users SET date_of_exit = NULL, status = 'active',
...
def main() -> int:
    ...
    conn = db()
    if admin_id:
        ensure_open_employment(conn, admin_id)
