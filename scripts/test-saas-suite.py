...
def main() -> int:
    ...
    if pt:
        ...
        if len(org_list) < 2:
            # Seed a second trial tenant so isolation cases have a real target.
            code_create, created = http(
                "POST",
                f"{API}/api/platform/organizations",
                {
                    "name": f"SaaS Trial Org {datetime.now().strftime('%H%M%S')}",
                    "slug": f"saas-trial-{datetime.now().strftime('%H%M%S')}",
                    "admin_name": "Trial Admin",
                    "admin_email": f"saas.trial.{datetime.now().strftime('%H%M%S')}@example.com",
                    "admin_password": "LocalTest123!",
                    "plan": "trial",
                },
                ph,
            )
            if code_create in (200, 201):
                code, orgs = http("GET", f"{API}/api/platform/organizations", headers=ph)
                org_list = orgs.get("data", []) if isinstance(orgs, dict) else []
        suite.record(
            "SAAS-05",
        ...
        second_org_id = next((o.get("id") for o in org_list if o.get("id") not in (None, 1)), None)
    ...
    # Inject punch on org1 device (ensure PIN→user mapping first)
    mapped_uid = ensure_demo_employee_1(t1) if t1 else None
    if not mapped_uid and t1:
        mapped_uid = ensure_demo_employee_1(t1)
    t_stamp = f"{TEST_DAY} 09:15:00"
    conn = db_connect()
    conn.execute(
        "DELETE FROM biometric_punches WHERE punch_time=? AND device_serial=?",
        (t_stamp, SN_ORG1),
    )
    conn.commit()
    ...
    suite.record(
        "SAAS-24",
        "Device punch stored for mapped tenant user",
        row is not None and mapped_uid is not None and row[0] == mapped_uid,
        f"user_id={row[0] if row else None} expected={mapped_uid}",
    )
    ...
    if pt:
        ph = {"Authorization": f"Bearer {pt}"}
        org2_id = second_org_id
        if org2_id is None:
            code, orgs = http("GET", f"{API}/api/platform/organizations", headers=ph)
            org_list = orgs.get("data", []) if isinstance(orgs, dict) else []
            org2_id = next((o.get("id") for o in org_list if o.get("id") not in (None, 1)), None)
        if org2_id is None:
            suite.record(
                "SAAS-26",
                "Platform impersonate org 2 (trial tenant)",
                False,
                "no second org available",
            )
        else:
            code, imp2 = http(
                "POST",
                f"{API}/api/platform/organizations/{org2_id}/impersonate",
                headers=ph,
            )
            ...
            suite.record(
                ...
                f"org={org2_id} HTTP {code}" + (f" ({err_msg})" if err_msg else ""),
            )
        ...
        if t2:
            ...
            # Trial/starter plans may return 403 when biometric module is not enabled.
            punches_ok = not leaked and code in (200, 403, 404)
            suite.record(
                "SAAS-27",
                "Org2 cannot see org1 biometric punches",
                punches_ok,
                f"org2_punches={len(p2_data)} HTTP {code}",
            )
            ...
            devices_ok = not has_org1_device and code in (200, 403, 404)
            suite.record(
                "SAAS-28",
                "Org2 device list excludes org1 hardware",
                devices_ok,
                f"devices={len(d2_data)} HTTP {code}",
            )
