...
from test_helpers import TENANT_LOGIN as HELPER_LOGIN, PLATFORM_LOGIN as HELPER_PLATFORM

TENANT_LOGIN = {
    "email": os.environ.get("HRM_EMAIL", HELPER_LOGIN["email"]),
    "password": os.environ.get("HRM_PASSWORD", HELPER_LOGIN["password"]),
    "org_slug": os.environ.get("HRM_ORG", HELPER_LOGIN["org_slug"]),
}
PLATFORM_LOGIN = {
    "email": os.environ.get("PLATFORM_ADMIN_EMAIL", HELPER_PLATFORM["email"]),
    "password": os.environ.get("PLATFORM_ADMIN_PASSWORD", HELPER_PLATFORM["password"]),
}
...
def main() -> int:
    ...
    if rls_on and pg_url.startswith("postgres"):
        ...
        # Prefer API-level deny for a non-existent user id (works on Postgres local).
        code, _ = http(
            "GET",
            f"{API}/api/admin/users/999999999",
            headers={"Authorization": f"Bearer {tenant_token}"},
        ...
        suite.record(
            ...
            code in (403, 404),
            f"missing-user HTTP {code}",
        )

    # Storage IDOR / traversal matrix
    for idx, tail in enumerate(
        [
            "users/00000000-0000-0000-0000-000000000000.jpg",
            "chat/does-not-exist.bin",
            "../etc/passwd",
        ],
        start=18,
    ):
        ...
        suite.record(
            f"SEC-{idx}",
            f"Storage path denied `{tail}`",
            code in (400, 403, 404),
            f"HTTP {code}",
        )

    # 2FA status endpoint (smoke; full enable/disable in test-gap-coverage-suite)
    code, _ = http(
        "GET",
        f"{API}/api/two-factor/status",
        headers={"Authorization": f"Bearer {tenant_token}"},
    )
    suite.record("SEC-21", "Two-factor status reachable", code == 200, f"HTTP {code}")
    ...
    suite.record("SEC-22", "Two-factor reject bad code", code in (400, 401, 422), f"HTTP {code}")
