...
def main() -> int:
    ...
    mapped_uid = conn.execute(
        "SELECT user_id FROM biometric_user_map WHERE device_serial=? AND device_pin='1'", (SN,)
    ).fetchone()[0]
    ...
    suite.record(
        ...
        sess is not None and sess["clock_in"] == "09:00:00",
    ...
    sess = open_session(conn, mapped_uid, TEST_DAY)
    suite.record("TC-12", "Re-check-in opens new session", sess is not None and sess["clock_in"] == "10:00:00")
    ...
    suite.record(
        ...
        guru_open is not None and guru_open["clock_in"] == "14:30:00",
