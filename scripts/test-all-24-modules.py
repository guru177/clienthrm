#!/usr/bin/env python3
"""Test all tenant modules from backend plan_limits::MODULE_CATALOG (API + catalog sync)."""
...
# Matches backend/src/plan_limits.rs MODULE_CATALOG.
MODULE_CATALOG: list[tuple[str, str, str]] = [
    ...
    ("doctor_reports", "Doctor Reports", "/api/admin/doctor-reports"),
    ("my_doctor_reports", "My Doctor Reports", "/api/admin/me/doctor-reports"),
    ("grocery_benefits", "Grocery Benefits", "/api/admin/grocery-benefits"),
    ("my_grocery_benefits", "My Grocery Benefits", "/api/admin/grocery-benefits/my-status"),
    ("assets", "Assets & Maintenance", "/api/admin/assets"),
    ("my_assets", "My Assets", "/api/admin/my-assets"),
...
UI_ROUTES: list[tuple[str, str]] = [
    ...
    ("doctor_reports", "/admin/doctor-reports"),
    ("my_doctor_reports", "/admin/my-doctor-reports"),
    ("grocery_benefits", "/admin/grocery-benefits"),
    ("my_grocery_benefits", "/admin/my-grocery-benefits"),
    ("assets", "/admin/assets"),
    ("my_assets", "/admin/my-assets"),
...
class Suite:
    ...
    def summary(self) -> int:
        ...
        print("\n" + "=" * 60)
        print(f"TENANT MODULE RESULTS: {passed}/{total} passed")
        if passed < total:
            print("\nFailed:")
...
def main() -> int:
    ...
    suite.record(
        "M0-01",
        f"Catalog defines exactly {EXPECTED_MODULE_COUNT} modules",
        len(MODULE_CATALOG) == EXPECTED_MODULE_COUNT,
        f"count={len(MODULE_CATALOG)}",
    )
    suite.record(
        "M0-02",
        "UI route map covers all tenant modules",
        len(UI_ROUTES) == EXPECTED_MODULE_COUNT
        and {k for k, _ in UI_ROUTES} == {m[0] for m in MODULE_CATALOG},
    ...
    if pt:
        ...
        suite.record(
            "M0-03",
            "Platform modules catalog API returns expected keys",
            code == 200 and len(keys) == EXPECTED_MODULE_COUNT,
