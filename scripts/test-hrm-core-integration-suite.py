...
def ensure_demo_employee_ready(conn: sqlite3.Connection, user_id: int) -> None:
    """Open employment window and clear exit so integration leave/attendance dates work."""
    conn.execute(
...
def main() -> int:
    ...
    if not emp:
        ...
        return suite.summary()
    user_id = int(emp["id"])
    ensure_demo_employee_ready(conn, user_id)
    ...
    code, demo_login = http(
        ...
        {"email": demo_email, "password": os.environ.get("HRM_PASSWORD", "Guru!1234"), "org_slug": "mashuptech"},
    )
    demo_token = None
    demo_auth = auth
    ...
    june_leave_start, june_leave_end = "2026-06-22", "2026-06-23"
    demo_leave_id = None
    if demo_token:
        code, dlv = http(
            "POST",
        ...
        if not demo_leave_id:
            june_leave_start, june_leave_end = "2026-06-17", "2026-06-18"
            code, dlv = http(
                "POST",
                f"{API}/api/admin/leave-requests",
                {
